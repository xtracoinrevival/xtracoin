...
const int64 nChainStartTime = 1400330510; 
...
const string strMessageMagic = "xtracoin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    // xtracoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
	...
	if(pindexBest->nHeight > 150000)
     //   nRewardCoinYear = 2.6 * MAX_MINT_PROOF_OF_STAKE/500 ; //this will delete currently 0.23 *500  will convert to  /500 * 2.6 this wil make 60% tearly
		 nRewardCoinYear = NEW_MAX_MINT_PROOF_OF_STAKE;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal xtracoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            if (nStakeReward > GetProofOfStakeReward(nCoinAge, pindexBlock->nBits, pindexBlock->nHeight, nTime) - GetMinFee() + MIN_TX_FEE)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in xtracoin
    bool fStrictPayToScriptHash = true; // Always active in xtracoin
    ...
    // ppcoin: fees are not collected by miners as in xtracoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "xtracoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        txNew.strTxComment = "text:xtracoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x99e5b0d12869287a52305e343f2af3981cc337a56699a41646e4b1383e4ffc7f"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("xtracoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("xtracoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("xtracoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("xtracoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadxtracoinMiner(void* parg);

static bool fGeneratextracoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void xtracoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("xtracoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneratextracoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratextracoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running xtracoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadxtracoinMiner(void* parg)
{
    ...
    try
    {
        ...
        xtracoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadxtracoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadxtracoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadxtracoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatextracoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratextracoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratextracoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d xtracoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadxtracoinMiner, pwallet))
                printf("Error: NewThread(ThreadxtracoinMiner) failed\n");
            Sleep(10);
        }
    }
}
