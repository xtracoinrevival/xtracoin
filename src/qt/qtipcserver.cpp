...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "xtracoin:"))
        {
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any xtracoin: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one xtracoin instance is listening
        message_queue::remove(BOUNTYCOINURI_QUEUE_NAME);
        delete mq;
